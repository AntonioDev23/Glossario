...
const glossario = {
  ...
  function adicionarTermo() {
    // Obtém o valor do campo de input e remove espaços em branco no início e no fim
    const termo = document.getElementById('termo').value.trim();
  
    // Verifica se o campo de input está vazio
    if (termo === '') return;
  
    // Busca a definição do termo no glossário
    const definicao = buscarDefinicao(termo);
  
    // Cria uma string com o termo e sua definição
    const termoCompleto = `${termo}: ${definicao}`;
  
    // Obtém a lista de termos do DOM
    const listaTermos = document.getElementById('listaTermos');
  
    // Cria um novo elemento <li> para o termo
    const novoTermo = document.createElement('li');
    novoTermo.textContent = termoCompleto;
  
    // Adiciona o novo termo à lista
  ...
  function buscarDefinicao(termo) {
    // Converte o termo para minúsculas para facilitar a busca
    const termoFormatado = termo.toLowerCase();
  
    // Verifica se o termo existe no glossário
    if (glossario[termoFormatado]) {
      // Retorna a definição do termo
      return glossario[termoFormatado];
    } else {
      // Retorna uma mensagem de erro se o termo não for encontrado
      return "Este termo não está no glossário de programação. Tente palavras como 'loop', 'function' ou 'variable'.";
    }
  }
  
  // Função para carregar os termos salvos no localStorage ao iniciar a página
  window.onload = function () {
    // Obtém os termos salvos do localStorage
    const termosSalvos = JSON.parse(localStorage.getItem('glossario')) || [];
  
    // Obtém a lista de termos do DOM
    const listaTermos = document.getElementById('listaTermos');
  
    // Adiciona cada termo salvo à lista
    termosSalvos.forEach(termo => {
      const novoTermo = document.createElement('li');
      novoTermo.textContent = termo;
      listaTermos.appendChild(novoTermo);
    });
  };